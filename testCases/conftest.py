...
def pytest_addoption(parser):
    ...
    admin_email = os.environ.get("CONDUIT_TEST_EMAIL")
    parser.addoption("--email", default=admin_email,
                     help=f"Email of the registered test user")

    admin_password = os.environ.get("CONDUIT_TEST_PASSWORD")
...
def email(request):
    ...
    if not adminemail_from_cli:
        ...
        os.environ['CONDUIT_TEST_EMAIL'] = email
        return email
    os.environ['CONDUIT_TEST_EMAIL'] = adminemail_from_cli
...
def password(request, email):
    ...
    if not password_from_cli:
        ...
        if not password:
            pytest.exit("Password for test user not provided. Please use --password to provide the password", 1)
        os.environ["CONDUIT_TEST_PASSWORD"] = password
        return password
    with open("cred", "w") as file:
        file.writelines([email, "\n", password_from_cli])
    os.environ["CONDUIT_TEST_PASSWORD"] = password_from_cli
...
def pytest_configure(config):
    config._metadata['Project Name'] = 'Conduit Test App'
    config._metadata['Module Name'] = 'Articles'
    config._metadata['Tester'] = 'Siddharth'


def pytest_html_report_title(report):
    report.title = "Conduit - UI Tests"
